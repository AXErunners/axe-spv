const { MerkleBlock, Transaction } = require('@axerunners/axecore-lib');
...
describe('SPV-AXE (addHeaders) add testnet headers out of order', () => {
  before(() => {
    chain = new Blockchain('testnet', 10000, utils.normalizeHeader(testnet[0]));
  });
...
describe('SPV-AXE (addHeaders) add mainnet headers out of order', () => {
  before(() => {
    chain = new Blockchain('mainnet', 10000, utils.normalizeHeader(mainnet[0]));
  });
